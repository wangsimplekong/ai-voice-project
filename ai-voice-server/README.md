...
┌─────────────────────────────────────────┐
│              前端 / 客户端               │
│  WS /api/voice/asr/realtime   REST /api │
└─────┬──────────────────────────┬────────┘
      │                          │
┌─────▼──────────┐  ┌───────────▼──────────┐
...
└─────┬──────────┘  └───┬──────────┬───────┘
      │                 │          │
┌─────▼─────────────────▼──┐ ┌────▼──────────┐
│  IAsrEngine (实时路由)    │ │ IAsrFileEngine │ ITtsEngine
│  ┌───────────────────┐   │ │ (文件路由)      │
│  │DashScopeAsrEngine │   │ │ ┌────────────┐ │
│  │ ├─ OmniRealtime   │   │ │ │Recognition │ │
│  │ ├─ Recognition    │   │ │ │.call(File) │ │
│  │ ├─ GummyRealtime  │   │ │ ├────────────┤ │
│  │ └─ GummyChat      │   │ │ │Transcription│ │
│  └───────────────────┘   │ │ ├────────────┤ │
└──────────────────────────┘ │ │QwenTranscr.│ │
                             │ ├────────────┤ │
  ITextCorrectionService     │ │MultiModal  │ │
  → qwen-turbo 纠错          │ └────────────┘ │
                             └────────────────┘
```

**本地模式**（`local.enabled=true`）：由 Local 实现接管，请求转发至自部署的 qwen-asr-serve、asr_realtime_server、tts_server（见 `voice-server/`）。
...
ai:
  voice:
    local:
      ...
      tts-voice: vivian                # 可选: aiden, dylan, eric, ono_anna, ryan, serena, sohee, uncle_fu, vivian
      timeout-ms: 30000
```

本地需先部署 `voice-server/` 下的 qwen-asr-serve、asr_realtime_server、tts_server（见项目内 `voice_manager.sh`）。
...
- **失败**：HTTP 200 + body 为统一封装 `R`（如 `{ "code": 500, "msg": "语音合成失败..." }`）。
...
失败时 `code` 非 0，`msg` 为错误说明，`data` 可为 null。
